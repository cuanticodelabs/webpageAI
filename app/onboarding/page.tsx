...
export default function OnboardingPage() {
  ...
  const initializeN8nChat = () => {
    ...
    try {
      ...
      createChat({
        ...
        initialMessages: [
          lang === 'es'
            ? 'Mi nombre es Teoh. ¿Cómo podemos potenciar tu negocio el día de hoy?'
            : 'My name is Teoh. How can we boost your business today?'
        ],
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <style>{`
        /* Ocultar powered by n8n y header */
        .n8n-chat__powered-by, [class*="powered"], a[href*="n8n.io"], .chat-powered-by { display: none !important; }
        #n8n-chat-container .chat-header { display: none !important; }

        /* Fondo del chat */
        #n8n-chat-container,
        #n8n-chat-container .n8n-chat,
        #n8n-chat-container .chat-messages-list,
        #n8n-chat-container .chat-body { background: #f8fafc !important; height: 100% !important; }

        /* Layout interno del widget ocupa todo el alto */
        #n8n-chat-container .n8n-chat { display: flex !important; flex-direction: column !important; height: 100% !important; }
        #n8n-chat-container .chat-messages-list { flex: 1 !important; overflow-y: auto !important; padding: 20px 16px !important; }

        /* Burbujas IA — navy, letras blancas */
        #n8n-chat-container .chat-message--from-bot .chat-message-bubble {
          ...
          box-shadow: 0 2px 8px rgba(10,31,56,0.18) !important;
          border: none !important;
        }

        /* Burbujas usuario — azul marino */
        #n8n-chat-container .chat-message--from-user .chat-message-bubble {
          ...
          line-height: 1.6 !important;
          max-width: 75% !important;
          box-shadow: 0 2px 8px rgba(10,31,56,0.30) !important;
          border: none !important;
        }

        /* Alineación mensajes */
        #n8n-chat-container .chat-message--from-user { justify-content: flex-end !important; display: flex !important; }
        #n8n-chat-container .chat-message--from-bot { justify-content: flex-start !important; display: flex !important; }
        #n8n-chat-container .chat-message { margin-bottom: 12px !important; display: flex !important; }

        /* Input area — blanca limpia */
        ...
        #n8n-chat-container .chat-input textarea:focus {
          border-color: #1e3a5f !important;
          outline: none !important;
          box-shadow: 0 0 0 3px rgba(10,31,56,0.12) !important;
        }
        #n8n-chat-container .chat-input textarea::placeholder { color: #94a3b8 !important; }

        /* Botón enviar — azul marino */
        #n8n-chat-container .chat-input button[type="submit"],
        #n8n-chat-container button.send-button {
          background: #1e3a5f !important;
          color: #ffffff !important;
        ...
        #n8n-chat-container .chat-input button[type="submit"]:hover { background: #0a1f38 !important; }

        /* Typing dots */
        #n8n-chat-container .chat-message--typing .chat-message-bubble { background: #0a1f38 !important; }
        #n8n-chat-container .chat-message--typing span { background: #2dd4bf !important; }

        /* Scroll */
        #n8n-chat-container ::-webkit-scrollbar { width: 4px; }
        #n8n-chat-container ::-webkit-scrollbar-track { background: transparent; }
        #n8n-chat-container ::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 4px; }
      `}</style>
      {/* Header */}
      ...
      <div className={`mx-auto px-6 pb-12 ${currentPhase === 3 ? 'max-w-4xl' : 'max-w-2xl'}`}>
        ...
        {currentPhase === 3 && (
          <Card className="border border-gray-200 shadow-xl rounded-2xl overflow-hidden flex flex-col" style={{height: 'calc(100vh - 180px)', minHeight: '600px'}}>
            {/* Header tipo ChatGPT */}
            <div className="flex items-center gap-3 px-5 py-4 border-b border-gray-100 bg-white flex-shrink-0">
              <div className="flex items-center justify-center h-8 w-8 rounded-full bg-[#0a1f38]">
                <span className="text-teal-400 font-bold text-xs">T</span>
              </div>
              <div>
                <p className="font-semibold text-[#0a1f38] text-sm leading-none">Teoh</p>
                <p className="text-xs text-emerald-500 mt-0.5">{lang === 'es' ? 'Asistente IA · En línea' : 'AI Assistant · Online'}</p>
              </div>
            ...
            {/* Área del chat - ocupa todo el espacio disponible */}
            <div className="flex-1 relative overflow-hidden">
              {!chatInitialized && (
                <div className="absolute inset-0 bg-white flex items-center justify-center z-10">
                  <div className="text-center space-y-3">
                    <div className="animate-spin rounded-full h-8 w-8 border-2 border-[#0a1f38] border-t-transparent mx-auto" />
                    <p className="text-gray-400 text-sm">{lang === 'es' ? 'Iniciando chat...' : 'Starting chat...'}</p>
                  </div>
                </div>
              )}
              <div
                ...
                style={{
                  ['--chat--color-primary' as any]: '#1e3a5f',
                  ['--chat--color-primary-shade-50' as any]: '#047857',
                  ['--chat--color-primary-shade-100' as any]: '#065f46',
                  ['--chat--color-secondary' as any]: '#0a1f38',
                  ['--chat--color-secondary-shade-50' as any]: '#0d2a4a',
                  ['--chat--background' as any]: '#f8fafc',
                  ['--chat--border-radius' as any]: '16px',
                  ['--chat--textarea--height' as any]: '44px',
                }}
              />
            </div>

            {/* Botón finalizar - ancho completo, azul marino */}
            <div className="flex-shrink-0 border-t border-gray-100 bg-white p-0">
              <Button
                onClick={handleFinishPhase3}
                disabled={!chatInitialized || isFinishing}
                className="w-full rounded-none rounded-b-2xl bg-[#1e3a5f] hover:bg-[#0a1f38] text-white font-semibold py-4 text-base transition-colors"
              >
